...
static void call_lupe_lua(struct lupe_lua lua, enum lupe_mode mode, int argc, char **argv) {
	char **args = c8l_arrnew(*args,10);
	register uint i=1;
	/* arguments passed to Lua */
	c8l_arrpush(args, lua.bin);
	c8l_arrpush(args, "-l");
	c8l_arrpush(args, mode == lupe_mode_cli ? "lupe.cli" : "lupe.dir");
	c8l_arrpush(args, "--");
	c8l_arrpush(args, mode == lupe_mode_cli ? "/dev/null" : lupe_script);

	for(; i < argc; i++) c8l_arrpush(args,argv[i]);

	if (!c8l_arrpush(args, NULL)) {
		puts(strerror(errno));
		exit(1);
	}
	lupe_env(lua, argc, argv);
	execvp(args[0], args);
	lupe_error("'exec %s': %s", args[0], strerror(errno));
}
